...
As an alternative to native Windows builds with Microsoft Visual Studio, thermos
also supports building on Windows systems with [MSYS2](https://www.msys2.org/).
MSYS2 provides some typical GNU / Linux tools on Windows operating systems.
Those can be used to build thermos.
...
To build the thermos from source you need a C++ compiler, CMake, the Ninja
build system, and the SQLite 3 library.
...
```bash
pacman -S git mingw-w64-x86_64-catch mingw-w64-x86_64-cmake mingw-w64-x86_64-gcc mingw-w64-x86_64-ninja mingw-w64-x86_64-pkg-config mingw-w64-x86_64-sqlite3
```
