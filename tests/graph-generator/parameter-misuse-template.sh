...
"$EXECUTABLE" --template /tmp/foo.tpl --template /tmp/bar.tpl --file /tmp/foo.db --output /tmp/foo.html
if [ $? -ne 1 ]
then
...
"$EXECUTABLE" --file /tmp/foo.db --output /tmp/foo.html --template
if [ $? -ne 1 ]
then
  echo "Executable did not exit with code 1."
  exit 1
...
"$EXECUTABLE" --file /tmp/foo.db --output /tmp/foo.html
if [ $? -ne 1 ]
then
  echo "Executable did not exit with code 1."
  exit 1
